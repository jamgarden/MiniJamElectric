...
func _interpret_bindings_column_types(_type_string_array: Array) -> void:
	bindings_column_types = []
	var i = 0
	for _type_string in _type_string_array:
		i += 1
		if i > bindings_per_action:
			break
		if _type_string == "any" or _type_string == "*":
			bindings_column_types.append(BINDINGS_TYPE_TO_TYPE_NAME.keys())
		else:
			var parts = _type_string.split("|")
			var binding_array := []
			for part in parts:
